...
-- Exact Ice Hub pattern: UICorner already on Main above, now UIStroke on Main
-- UIGradient on the stroke, Offset animated = curved animated border
...
-- SIDEBAR
-- ================================================
-- Strategy: full UICorner rounds all 4 corners. Patch top-RIGHT and bottom-RIGHT
-- back to square with same-colored frames so only the left edge curves.
-- This gives the vertical bar curved top-left and bottom-left endpoints (matching the reference pic).
...
-- Patch top-right corner back to square (flush with sidebar colour)
...
-- Patch bottom-right corner back to square.
-- Two layers: outer = C.bg (exposes Main's bottom-left curve), inner = C.sidebar (fills the sidebar rectangle)
do local f=Instance.new("Frame")
   f.Size=UDim2.new(0,CORNER+2,0,CORNER+2)
   f.Position=UDim2.new(1,-(CORNER+2),1,-(CORNER+2))
   f.BackgroundColor3=C.bg; f.BorderSizePixel=0; f.ZIndex=4; f.Parent=Sidebar end
do local f=Instance.new("Frame")
   f.Size=UDim2.new(0,CORNER+1,0,CORNER+1)
   f.Position=UDim2.new(1,-(CORNER+2),1,-(CORNER+2))
   f.BackgroundColor3=C.sidebar; f.BorderSizePixel=0; f.ZIndex=5; f.Parent=Sidebar end
-- Additional cover: a C.bg frame flush at the very bottom-right to cut out the Main corner shape
do local f=Instance.new("Frame")
   f.Size=UDim2.new(0,CORNER,0,CORNER)
   f.Position=UDim2.new(1,-CORNER,1,-CORNER)
   f.BackgroundColor3=C.bg; f.BorderSizePixel=0; f.ZIndex=7; f.Parent=Main end

-- Vertical divider line — pill-shaped top cap, straight bottom
-- Starts below the top-left curve, runs all the way to the bottom of the sidebar
-- (bottom-right is patched square, so the line doesn't need to avoid a curve there)
local SBLine=Instance.new("Frame")
SBLine.Size=UDim2.new(0,1,1,-CORNER); SBLine.Position=UDim2.new(1,-1,0,CORNER)
SBLine.BackgroundColor3=Color3.fromRGB(45, 52, 75)
SBLine.BorderSizePixel=0; SBLine.ZIndex=10; SBLine.Parent=Sidebar
do local c=Instance.new("UICorner"); c.CornerRadius=UDim.new(1,0); c.Parent=SBLine end
...
TabList.Size=UDim2.new(1,-10,1,-(196+90))
...
-- ---- BOTTOM SIDEBAR IMAGE — fills ALL empty space between tab list bottom and footer ----
-- TabList ends at ~330px, footer is at H-FOOT_H=390px, so ~60px tall space
-- Fill the full width (minus small padding) AND full height of the available space
local tabListBottom = 196 + (H - 196 - 90)  -- = H - 90 = 330
local promoSpaceTop = tabListBottom + 2       -- 2px gap from tab list
local promoSpaceBot = H - FOOT_H - 2         -- 2px from footer
local promoSpaceH   = promoSpaceBot - promoSpaceTop
local promoPad      = 6                       -- padding on each side
local promoW        = SB_W - promoPad * 2    -- full width minus padding
-- Use full width × full available height — ScaleType.Fit handles aspect ratio internally
local promoX        = promoPad
local promoY        = promoSpaceTop

local PromoCard=Instance.new("ImageLabel")
PromoCard.Size=UDim2.new(0,promoW,0,promoSpaceH)
PromoCard.Position=UDim2.new(0,promoX,0,promoY)
PromoCard.BackgroundTransparency=1
PromoCard.Image="rbxassetid://133739031801572"
PromoCard.ScaleType=Enum.ScaleType.Fit
...
-- Patch bottom-left and bottom-right back to square
do local f=Instance.new("Frame")
   f.Size=UDim2.new(1,0,0,CORNER); f.Position=UDim2.new(0,0,1,-CORNER)
   f.BackgroundColor3=C.topbar; f.BorderSizePixel=0; f.ZIndex=5; f.Parent=TopBar end
...
local LABEL_Y = TOP_H + 6 + 110 + 4   -- 4px below banner bottom
local LABEL_H = 16

local TabNameLbl=Instance.new("TextLabel")
TabNameLbl.Size=UDim2.new(0,0,0,LABEL_H)   -- width auto-sized by text
...
TabNameLbl.TextColor3=C.sky; TabNameLbl.Font=Enum.Font.GothamBlack
...
-- Divider anchored to right of label via a container so it always starts 8px after the text
local TabDivContainer=Instance.new("Frame")
TabDivContainer.BackgroundTransparency=1; TabDivContainer.BorderSizePixel=0
TabDivContainer.Size=UDim2.new(1,-14,0,LABEL_H)
TabDivContainer.Position=UDim2.new(0,14,0,LABEL_Y)
TabDivContainer.ZIndex=6; TabDivContainer.Parent=RightPanel
do local l=Instance.new("UIListLayout")
   l.FillDirection=Enum.FillDirection.Horizontal
   l.VerticalAlignment=Enum.VerticalAlignment.Center
   l.Padding=UDim.new(0,8); l.Parent=TabDivContainer end

-- Invisible spacer — width synced to label every frame via Heartbeat
local TabDivSpacer=Instance.new("Frame")
TabDivSpacer.BackgroundTransparency=1; TabDivSpacer.BorderSizePixel=0
TabDivSpacer.Size=UDim2.new(0,0,1,0)   -- starts at 0; Heartbeat corrects it after first layout
TabDivSpacer.ZIndex=6; TabDivSpacer.Parent=TabDivContainer
...
TabDivLine.ZIndex=6; TabDivLine.Parent=TabDivContainer

-- Sync spacer width to label's actual rendered width every frame
-- task.defer sets the correct size after the first layout pass (avoids 1-frame flash at X=0)
task.defer(function()
    TabDivSpacer.Size=UDim2.new(0,TabNameLbl.AbsoluteSize.X,1,0)
end)
...
-- ---- CONTENT SCROLLFRAME ----
local contentTop = LABEL_Y + LABEL_H + 4   -- tight to name label
...
-- Patch top-left and top-right back to square
do local f=Instance.new("Frame")
   f.Size=UDim2.new(1,0,0,CORNER); f.Position=UDim2.new(0,0,0,0)
   f.BackgroundColor3=C.footer; f.BorderSizePixel=0; f.ZIndex=5; f.Parent=Footer end

...
local promoSpaceTop = tabListBottom + 2
local promoSpaceBot = H - FOOT_H - 2
local promoSpaceH   = promoSpaceBot - promoSpaceTop  -- ~58px

local PromoCard=Instance.new("ImageLabel")
PromoCard.AnchorPoint=Vector2.new(0.5,0)              -- centre horizontally
PromoCard.Size=UDim2.new(1,-8,0,promoSpaceH)          -- full width minus 4px each side
PromoCard.Position=UDim2.new(0.5,0,0,promoSpaceTop)   -- centred on sidebar
...
PromoCard.ScaleType=Enum.ScaleType.Crop               -- fills frame fully, no letterbox gap
...
-- DIVIDER LINE — fixed right edge (14px from panel right), left edge follows label width
-- Positioned on same vertical centre as the label text
local TabDivLine=Instance.new("Frame")
TabDivLine.Size=UDim2.new(1,0,0,1)   -- placeholder; updated every frame below
TabDivLine.Position=UDim2.new(0,14,0,LABEL_Y+7)
TabDivLine.BackgroundColor3=Color3.fromRGB(45,52,75); TabDivLine.BorderSizePixel=0
TabDivLine.ZIndex=6; TabDivLine.Parent=RightPanel

-- Every frame: left edge = label right + 6px gap, right edge = panel width - 14px
RunService.Heartbeat:Connect(function()
    local lblRight = 14 + TabNameLbl.AbsoluteSize.X   -- 14px left margin + text width
    local panelW   = RightPanel.AbsoluteSize.X
    local lineLeft = lblRight + 6                      -- 6px gap after text
    local lineW    = panelW - lineLeft - 14            -- 14px right margin
    if lineW > 0 then
        TabDivLine.Position = UDim2.new(0, lineLeft, 0, LABEL_Y+7)
        TabDivLine.Size     = UDim2.new(0, lineW,    0, 1)
    end
end)

...
-- Restore Main's bottom-left curve over the sidebar patch:
-- a C.bg rounded frame on Main cuts out the outer bottom-left corner of the GUI
do local f=Instance.new("Frame")
   f.Size=UDim2.new(0,CORNER+2,0,CORNER+2)
   f.Position=UDim2.new(0,0,1,-(CORNER+2))
...
TabList.Size=UDim2.new(1,-10,1,-(196+110))   -- 110px reserved at bottom for promo card
...
-- ---- PROMO CARD — fills the empty space between tab list and footer exactly ----
-- TabList bottom = H - 110 = 310, Footer top = H - FOOT_H = 390, gap = 80px
local TABLIST_BOTTOM = H - 110
local PROMO_TOP_GAP  = 4
local PROMO_BOT_GAP  = 4
local PROMO_H = (H - FOOT_H) - TABLIST_BOTTOM - PROMO_TOP_GAP - PROMO_BOT_GAP
...
PromoCard.ScaleType=Enum.ScaleType.Crop
...
-- CONTENT AREA
-- ================================================
-- RightPanel clips children to its rounded corners — TopBar and Footer curve naturally
...
-- Top bar inside right panel — plain frame, no UICorner needed
-- RightPanel already has UICorner; TopBar clips to it naturally via ClipsDescendants
...
-- DIVIDER LINE — sits below the label text, not through it
local TabDivLine=Instance.new("Frame")
TabDivLine.Size=UDim2.new(1,0,0,1)   -- placeholder; updated every frame below
TabDivLine.Position=UDim2.new(0,14,0,LABEL_Y+LABEL_H+3)
...
RunService.Heartbeat:Connect(function()
    ...
    if lineW > 0 then
        TabDivLine.Position = UDim2.new(0, lineLeft, 0, LABEL_Y+LABEL_H+3)
        TabDivLine.Size     = UDim2.new(0, lineW,    0, 1)
    end
end)

-- ---- CONTENT SCROLLFRAME ----
local contentTop = LABEL_Y + LABEL_H + 8   -- below label + divider
...
-- ---- FOOTER ----
-- Plain frame — RightPanel.ClipsDescendants=true handles the bottom corner rounding

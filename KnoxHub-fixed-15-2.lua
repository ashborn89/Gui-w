...
-- SIDEBAR
-- ================================================
-- UICorner on Sidebar gives curved top-left and bottom-left.
-- Right side is patched square by same-color frames.
-- Bottom-right corner of Main is separately preserved via a C.bg cutout on Main itself.
...
-- Patch top-right of Sidebar back to square (covers the UICorner curve on that corner)
...
-- Restore Main's bottom-left curve: place a C.bg rounded frame on Main that covers
-- only the very bottom-left corner area, cutting out the sidebar's square patch
do local f=Instance.new("Frame")
   f.Size=UDim2.new(0,CORNER+2,0,CORNER+2)
   f.Position=UDim2.new(0,0,1,-(CORNER+2))
   f.BackgroundColor3=C.bg; f.BorderSizePixel=0; f.ZIndex=8; f.Parent=Main
   local c=Instance.new("UICorner"); c.CornerRadius=UDim.new(0,CORNER); c.Parent=f end

-- Vertical divider line — runs full sidebar height minus both curve offsets
-- so the pill caps sit exactly inside the curved corners
local SBLine=Instance.new("Frame")
SBLine.Size=UDim2.new(0,1,1,-CORNER*2)
SBLine.Position=UDim2.new(1,-1,0,CORNER)
...
local promoSpaceBot = H - FOOT_H - 2         -- 2px above footer
local promoSpaceH   = promoSpaceBot - promoSpaceTop  -- ~58px tall

local PromoCard=Instance.new("ImageLabel")
PromoCard.Size=UDim2.new(1,-16,0,promoSpaceH)   -- full sidebar width minus 8px padding each side
PromoCard.Position=UDim2.new(0,8,0,promoSpaceTop)
...
PromoCard.ScaleType=Enum.ScaleType.Crop           -- Crop fills the frame fully, no letterbox
PromoCard.BorderSizePixel=0; PromoCard.ZIndex=3
PromoCard.ClipsDescendants=true
...
-- Divider anchored to right of label via a container — right edge has 14px margin to stay inside panel
local TabDivContainer=Instance.new("Frame")
TabDivContainer.BackgroundTransparency=1; TabDivContainer.BorderSizePixel=0
TabDivContainer.Size=UDim2.new(1,-28,0,LABEL_H)   -- 14px left margin + 14px right margin
...
do local l=Instance.new("UIListLayout")
   ...
   l.Padding=UDim.new(0,2); l.Parent=TabDivContainer end   -- 2px gap between text and line

...
-- Patch top-right of Sidebar back to square (internal corner only, does not reach outer border)
...
-- Bottom-right and bottom-left patches REMOVED.
-- They were causing rectangular artifacts (|_ shape) at the GUI corners by showing
-- through the transparent interior of BorderOverlay's rounded corners.
-- Sidebar now keeps its UICorner on all corners; the bottom-left aligns with Main's
-- outer curve and the bottom-right is internal/covered.
...
TabList.Size=UDim2.new(1,-10,1,-(196+110+20))       -- auto-fit: 196 header + 110 promo + 20 gap
...
-- ---- PROMO CARD — large, sits near bottom of sidebar ----
-- Tab list uses full remaining height; promo card floats near bottom edge.
local PROMO_H = 110                      -- taller for bigger logo
local PROMO_Y = H - FOOT_H - PROMO_H - 6 -- 6px gap above footer divider

local PromoCard=Instance.new("ImageLabel")
PromoCard.AnchorPoint=Vector2.new(0.5, 0)
PromoCard.Size=UDim2.new(1,-10,0,PROMO_H)
...
do local c=Instance.new("UICorner"); c.CornerRadius=UDim.new(0,12); c.Parent=PromoCard end
...
-- Footer top patch REMOVED.
-- It was causing a flat rectangular segment to show through the transparent interior
-- of BorderOverlay's rounded bottom corners. Footer keeps UICorner on all corners;
-- the top internal rounding is subtle and acceptable.
...
function makeToggle(parent,text,order,cb)
    ...
    -- Square toggle button with logo image and flying animation
    local TOG_SZ = 36
    local bg=Instance.new("Frame"); bg.Size=UDim2.new(0,TOG_SZ,0,TOG_SZ); bg.Position=UDim2.new(1,-(TOG_SZ+10),0.5,-TOG_SZ/2)
    ...
    -- Logo image larger than button so wings overflow
    local togLogo=Instance.new("ImageLabel")
    togLogo.Size=UDim2.new(0,TOG_SZ+18,0,TOG_SZ+14)
    togLogo.Position=UDim2.new(0,-9,0,-7)
    togLogo.BackgroundTransparency=1
    togLogo.Image="rbxassetid://120025299102610"
    togLogo.ScaleType=Enum.ScaleType.Fit
    togLogo.ZIndex=6; togLogo.Parent=bg

    -- Flying animation for toggle logo
    local togT=0
    local togConn=RunService.Heartbeat:Connect(function(dt)
        togT=togT+dt
        local s=1+math.sin(togT*4)*0.08
        local w=(TOG_SZ+18)*s
        local h=(TOG_SZ+14)*s
        local ox=-9-(w-(TOG_SZ+18))*0.5
        local oy=-7-(h-(TOG_SZ+14))*0.5
        local bob=math.sin(togT*3)*2.5
        local sway=math.sin(togT*1.8)*1
        togLogo.Size=UDim2.new(0,w,0,h)
        togLogo.Position=UDim2.new(0,ox+sway,0,oy+bob)
    end)
    table.insert(toggleConns, togConn)
...
-- Store toggle connections for cleanup
local toggleConns = {}
...
_G.KnoxHub_Cleanup = function()
    ...
    pcall(function() for _, c in ipairs(toggleConns) do pcall(function() c:Disconnect() end) end end)
